...
export function Sidebar({ activeTab, navItems, user, onNavigate, onLogout }: SidebarProps) {
  return (
    <aside className="w-64 border-r border-zinc-200 bg-white flex flex-col pt-8 shadow-sm z-30 shrink-0 select-none">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer",
                isActive
                  ? "bg-gradient-to-r from-[#00ADC6] to-[#018698] text-white shadow-md shadow-[#00ADC6]/20"
                  : "text-zinc-500 hover:text-zinc-800 hover:bg-zinc-100"
              )}
            >
              <Icon size={16} className={cn(isActive ? "text-white" : "text-zinc-400")} />
              {item.label}
            </button>
          );
        })}
        ...
        <div className="px-6 pb-2">
          <div className="bg-zinc-50 rounded-xl p-3 border border-zinc-100 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={onLogout}
                className="text-[10px] text-zinc-450 hover:text-rose-500 font-bold uppercase tracking-wider pl-2 transition-colors cursor-pointer"
                title="Sign Out"
              >
                Exit
              </button>
      ...
      <div className="p-6">
        <div className="bg-zinc-50 rounded-xl p-4 border border-zinc-100">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-1.5 h-1.5 rounded-full bg-[#00ADC6] animate-pulse" />
