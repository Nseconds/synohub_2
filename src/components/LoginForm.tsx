...
import { Shield, Lock, Activity, Eye, EyeOff, User } from "lucide-react";
...
export const LoginForm: React.FC<LoginFormProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden w-full" id="login_screen">
      {/* Background blobs */}
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-zinc-105 overflow-hidden relative z-10">
        {/* Header decoration */}
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-[#00ADC6] to-[#018698] hover:shadow-lg hover:shadow-[#00ADC6]/20 text-white rounded-xl py-3.5 text-xs font-bold transition-all flex items-center justify-center gap-2 mt-2 cursor-pointer"
              id="login_submit_btn"
            >
              {loading ? (
            ...
            <div className="mt-5 p-3.5 bg-zinc-50 rounded-xl border border-zinc-200 text-[11px] text-zinc-600 space-y-2">
              ...
              <div className="space-y-1.5">
                <p className="leading-relaxed">
                  User accounts and passwords are managed on the secure database. Only registered database users have access to portal systems. (Use **admin** / **admin** to log in).
                </p>
              </div>
            </div>
          </div>
